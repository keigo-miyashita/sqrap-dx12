...
namespace sqrp
{
	...
	WorkGraph::WorkGraph(const Device& device, wstring name, StateObjectHandle stateObject, UINT maxInputRecords, UINT maxNodeInputs)
		: pDevice_(&device), name_(name)
	{
		...
		if (FAILED(result)) {
			throw runtime_error("Failed to QueryInterface for StateObjectProperties1 : " + to_string(result));
		}
		...
		if (FAILED(result)) {
			throw runtime_error("Failed to CreateCommittedResource for backingmemory : " + to_string(result));
		}
		...
		// NOTE : 
		// local root argument��g�������ꍇ�C�����ő���
		//
		auto stateObjectDesc = stateObject->GetStateObjectDesc();
		if (holds_alternative<StateObjectDesc::WorkGraphDesc>(stateObjectDesc.typeDesc_)) {
			auto workGraphDesc = get<StateObjectDesc::WorkGraphDesc>(stateObjectDesc.typeDesc_);
